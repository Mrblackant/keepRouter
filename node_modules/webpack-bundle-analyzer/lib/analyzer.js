...
function getViewerData(bundleStats, bundleDir, opts) {
  ...
  var assets = _.transform(bundleStats.assets, function (result, statAsset) {
    ...
    asset.modules = _(modules).filter(function (statModule) {
      return assetHasModule(statAsset, statModule, parsedModules);
...
function assetHasModule(statAsset, statModule, parsedModules) {
  ...
  var moduleIsInsideAsset = _.some(statModule.chunks, function (moduleChunk) {
    return _.includes(statAsset.chunks, moduleChunk);
  });

  return moduleIsInsideAsset && (
  // ...and that we have found it during parsing
  !parsedModules || _.has(parsedModules, statModule.id));
}
